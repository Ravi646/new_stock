        ...
        var format = function(number,n, x){
            if(settings.format) {
                if (typeof number == 'number') {
                    ...
                    if(number >= 10000000) number = (number/10000000).toFixed(2) + ' Cr';
                    else if(number >= 100000) number = (number/100000).toFixed(2) + ' Lac';
                    else if(number >= 1000) number  = number.toFixed(Math.max(0, ~~n)).replace(new RegExp(re, 'g'), '$&,');
                    return number;
                }else{
                    // alert(number);
                    return number;
        ...
        return this.each(function () {
            ...
            var disableMaxLowerValues = function () {
                $popoverObj.find('.maximum-items li').each(function(index){
                    if(parseInt($(this).data('value')) < parseInt(minimumItemValue) || parseInt($(this).data('value')) == parseInt(minimumItemValue)) {
                        $(this).addClass('disabled').css("display", "none");
                    } else {
                        $(this).removeClass('disabled').css("display", "");
                    }
                });
            }
             ...
             */
            var disableCustomMaxLowerValues = function(){
                $popoverObj.find('.maximum-items li').each(function(index){
                     $(this).addClass('disabled').css("display", "none");
                });
            }
            ...
            $(this).popover({
                ...
                content: function () {
                    ...
                    var defaultValue = (settings.minValues[0] == 5000) ? "1000" : "100000";
                    var defaultValueResult = (settings.minValues[0] == 5000) ? format(5000) : format(500000);
                    var defaultValueMax = (settings.minValues[0] == 5000) ? "1000001": "1000000001";
                    var defaultValueMaxResult = (settings.minValues[0] == 5000) ? format(1000001) : format(1000000001);

                   //prepare minimum items
                    minItems+='<ul class="list-unstyled"><li data-value="'+defaultValue+'">Below '+format(defaultValueResult)+'</li>';
                    for(var i=0;i<settings.minValues.length;i++) {
                        minItems+='<li data-value="'+settings.minValues[i]+'">'+format(settings.minValues[i])+'</li>';
                    }
                    minItems+='<li data-value="'+defaultValueMax+'">Above '+defaultValueMaxResult+'</li></ul>';

                    //prepare maximum items
                    maxItems+='<ul class="list-unstyled"><li data-value="'+defaultValue+'">Below '+format(defaultValueResult)+'</li>';
                    for(var i=0;i<settings.maxValues.length;i++) {
                        maxItems+='<li data-value="'+settings.maxValues[i]+'">'+format(settings.maxValues[i])+'</li>';
                    }
                    maxItems+='<li data-value="'+defaultValueMax+'">Above '+defaultValueMaxResult+'</li></ul>';
